...
The explanations are meant to give you some intuition about them if you are not at all familiar with them.
</p>

<h3>Refinement types</h3>
<p>A <Link href="https://en.wikipedia.org/wiki/Refinement_type">refinement type</Link> is a type with a a predicate attached to it that holds for every value of this type.
</p>
<p>
Rust is not a language that has user-definable refinement types, but you can find some examples of refinements in the standard library, like <code>NonZero</code> and <code>NonNull</code>.
The <Link href="https://doc.rust-lang.org/std/num/struct.NonZero.html">documentation for NonZero</Link> mentions that <code>NonZero</code> can result in memory optimization.
For example, <code>{esc('Option<NonZero<u32>>')}</code> is the same size as <code>u32</code> because the compiler is able to use 0 to mean <code>None</code>. 
...
<p>
<Link href="https://github.com/google/wuffs">Wuffs</Link> is a memory-safe programming language that does have refinement types.
...
All these checks are done at <em>compile time</em>.
...
<p>
<Link href="https://en.wikipedia.org/wiki/Flow-sensitive_typing">Flow-sensitive typing</Link> (or flow typing) is something I came into contact with at work using <Link href="https://www.typescriptlang.org/">TypeScript</Link>.
...
<p>
There is a <Link href="https://morepypy.blogspot.com/2015/01/faster-more-memory-efficient-and-more.html">PyPy blog post</Link> explaining their then new dictionary design (it's a post from 2015).
Their design is not important here, just know that it uses an array of integers (<code>sparse_array</code>) as indices into another array (<code>compact_array</code>).
The value of the indices can only be as large as the size of the dictionary (<code>num_items</code>).
...
<p>
This means that the <em>actual</em> type (a 8-bit, 16-bit or 32-bit integer) of <code>variable_int</code> depends on the value of <code>num_items</code>.
...
<p>
In the example the <code>dict</code> comes in three variaties with either 8-bit, 16-bit or 32-bit indices.
You can differentiate between the three by inspecting the value of <code>num_items</code>.
The syntax of <code>u32(lo..hi)</code> is a meant as a refinement on <code>u32</code>, where its value must lie in the range <code>lo..hi</code>.
The double star <code>**</code> means "to the power of", so <code>2**8</code> is 256.
</p>
...
with a <code>small_dict</code> without having done the necessary checks.
</p>
...
The example below is for Rust and shows creating a <code>Vec</code> with a capacity of 8.
A few elements are pushed, a reference to an element is taken and then an attempt is made to push again.
</p>

<CodeBlock lang="rust" code={`
...
<p>
In Rust it is illegal to hold a reference to a <code>Vec</code> element and then push an element.
You are holding an immutable reference and then try to mutably borrow it.
The reason you need to mutably borrow the <code>Vec</code> for a push is that the underlying data where the elements are stored might move.
If a <code>Vec</code> has no space left for a new element it has to move the elements to a place with more room for new elements.
</p>

<p>
However, in this example the push will not result in such a relocation so the reference will still be valid afterwards. 
Even if you check that the <code>Vec</code> has spare room you are not allowed to push while holding a reference.
Annoying!
</p>

<p>
If you could overload a function based on a refinement, then you could define a version of <code>push</code> that does not require a mutable borrow,
...
<p>
Even if overloading would not be possible you could still define another version of <code>push</code> that only can be called if you know that there is enough room.
</p>
...
In the pseudocode <code>address</code> is an implicitly available variable that represents the address of a value of the type. 
</p>
...
`} />
  <p>
If the program is well typed, you could never accidentally read or write outside of VRAM when using a <code>VRAMPointer</code>.
  </p>
...
Or to be sure that indexing an array will not be out of bounds you need to check the index value. <
  s>For this last case Rust enables out of bounds access checks for `Vec`s in debug builds, but disables them for release builds.</s> (This is not true 🙈 I got <Link href="https://news.ycombinator.com/item?id=43171996">corrected by kibwen</Link>)
</p>

<p>
What you could do is introduce a special function called <code>assume</code> that can be given a predicate and
...
For debug builds you can turn the calls to <code>assume</code> into <code>assert</code> and find the problem.
So, <code>assume</code> is essentially a <code>debug_assert</code> except that it also ties in to the flow typing of the language.
</p>
    </>
  );
};
